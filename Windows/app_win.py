...
def get_app_dir() -> Path:
    # Répertoire de l'exécutable si gelé, sinon répertoire du script
    if is_frozen():
...
def ensure_ffmpeg_available() -> Optional[str]:
    # 1) À côté de l'exécutable/script
    app_dir = get_app_dir()
    ...
    # 3) imageio-ffmpeg (auto-download); on renvoie le binaire téléchargé
    try:
        import imageio_ffmpeg as iio_ffmpeg
        bin_path = iio_ffmpeg.get_ffmpeg_exe()
        # Essayer de copier à côté de l'app pour une prochaine exécution
        target = app_dir / ("ffmpeg.exe" if os.name == "nt" else "ffmpeg")
...
def find_png_icon_path() -> Optional[str]:
    # Premier PNG trouvé à côté de l'app
    app_dir = get_app_dir()
    for png in app_dir.glob("*.png"):
        return str(png)
    return None
...
class YouTubeDownloaderGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("YT to me - Téléchargeur 720p (Windows)")
